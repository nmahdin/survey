...
//                    User::find($request->user()->id)->update(['random' => $new_qustions]);
                    $qustion_id = Questions::all()->pluck('id')->shuffle()->toArray();
                    $qustions_id = Questions::all()->pluck('id')->toArray();
                    if ($request->user()->random != null) {
                        $rand = explode('.', $request->user()->random);
                    } else {
                       $rand = [0];
                    }
...
//                    dd($qustion_id);
                    ...
                    for ($i = 0 ; $i < end($qustions_id) || $i < Questions::count(); $i++) {
                        if (! in_array($qustion_id[$i] , $rand)) {
                            $q = Questions::find($qustion_id[$i]);
                            array_push($rand , $qustion_id[$i]);

                            $rand = collect($rand)->implode('.');
                            $request->user()->update(['random' => $rand ]);
